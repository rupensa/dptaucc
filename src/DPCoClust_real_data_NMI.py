...
for n_iterations in iter_list:
    for dataset in datasets:
        ...
        fhd, date = CreateOutputFileDPHD(dataset, plusk=n_iterations)
        fnmf, date = CreateOutputFileDPNMF(dataset, plusk=n_iterations)
        ...
        for t in range(n_test):
            for eps in [.1, .2, .3, .4, .5, .6, .7, .8, .9, 0.99999, 1.5,2,2.5,3,4.5,5,6,7,8,9,10]:
                ...
                mat_file['file_n_iter'] = n_iterations
                mat_file['file_maxval'] = np.max(T).astype('float')
                ...
                execute_test_dphd(fhd, T, target, dataset, noise=0, n_iterations = n_iterations, eps = eps, init = k, verbose = False)
                if eps<1:
                    print(f'DPNMF on {dataset}, with epsilon {eps} and n. iter {n_iterations}, run {t}')
                    execute_test_dpnmf(fnmf, T, target, n_iterations=n_iterations, eps = eps, init = k, verbose = False)
